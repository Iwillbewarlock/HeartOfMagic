...
Not done yet, in order of expected gain: batching same-coloured edges and nodes into one path (dashed
locked edges alone are ~2,000 `setLineDash` calls a frame), drawing the simple tier while the view is
moving, trimming the 500-unit cull margin, and - separately, because it needs a plugin build and may
simply not work - trying the accelerated view.
...
three places are consulted in order: the browser's `localStorage` (`hom_language`, read in `index.html`),
then `locale.js`, then `settings.language` when the config arrives - which switches if it differs and
refreshes `localStorage` for next time. A live switch re-labels everything marked `data-i18n`; text a
script already built with `t()` stays until the next start, and the picker says so. **Not yet known:**
whether PrismaUI keeps `localStorage` between game sessions. If it does not, every start goes through the
late switch and script-built text stays in the default language; the fix then is to have C++ write the
choice where the page can read it synchronously.
