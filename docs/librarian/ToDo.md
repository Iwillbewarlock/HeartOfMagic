...
M0 · M1 · M2(a)(b) · M3 · M4 완료. **사서가 게임 안에서 돌고 카탈로그를 남긴다.**
게임이 쓴 카탈로그와 하네스가 만든 카탈로그가 1440건 전부 동일하다는 것까지 확인됐다.
...
**카탈로그를 읽는 쪽이 아직 없다.** 지금은 파일만 남긴다. 다음은 손님을 붙이는 일이고,
둘 다 게임 테스트가 필요하다.

1. **M6 (퍽 모드 호환 보정) 을 먼저 하는 쪽이 낫다.** Concept 8절이 말하는 "퍽 모드 호환"의
   실체이고, 사용자가 바로 체감한다. 빠진 바닐라 `Magic*` 키워드를 MGEF 에 채워 넣는 일이라
   태그 정확도가 낮아도 손해가 작다(추가만 하고 삭제하지 않는다)
2. **M5 (트리)** — 태그를 TF-IDF 특징량으로 주입. 트리 품질은 정성 평가라 판단이 오래 걸린다
3. **M7 (텍스트·LLM 폴백)** — 태그 0개가 (a) 18.6% / (b) 6.9%. 이펙트명 룰은 **언어 의존**이라
   한글 로드오더에서 안 먹는다. 영어 환경 보너스로만 취급할 것
4. 남은 잔가지: `RunScan` 이 세션에 따라 120초 걸리는 문제(M1 버그 2), 포커스 잃으면
   프레임 정지(M0-T). 둘 다 게임 테스트를 느리게 할 뿐 기능을 막지는 않는다
