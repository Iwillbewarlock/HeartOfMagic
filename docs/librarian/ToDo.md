...
### [~] S-MIX. 학파가 섞이는 트리 (2026-09-22 시작) — 학파 칸은 유지, 이웃 경계에서 가지가 넘나든다. 트리는 2D 유지
- [x] 1단계 다리 규칙 — `TreeBuilderBridges.cpp`, 빌더 5종 공통 출력 `bridges`. 1440주문에서 180개(양방향 44). 규칙·한계는 `docs/TREE_BUILDING_SYSTEM.md` "Cross School Bridges"
- [x] 2단계 배치 (2026-09-22, 브라우저 미리보기 검증, 게임 확인 대기) — `modules/schoolBridges.js`. 학파 순서는 C++ `schoolLinks`(상한 적용 전 친족 수)로 정하고, 다리의 `from`을 `to`의 softPrereqs에 추가, 적용된 다리를 저장본 `bridges`에 복사. 빌드 방식 5종 공통
  - 시도 후 철회: 다리 걸린 주문·테마를 경계 쪽으로 당기기. classic 배치는 부모를 따라 바큇살 방향으로 뻗어서 당김이 측정상 아무 효과 없었음 (다리 평균 길이 트리 반지름의 ~63% 그대로). 다리 선은 길 수밖에 없으므로 3단계에서 "평소엔 숨기고 선택/호버 때만" 으로 풀어야 함
- [x] 3단계 UI (2026-09-22, 브라우저 미리보기 검증, 게임 확인 대기) — `modules/bridgeView.js`: 선택/호버한 주문의 다리만 점선, 도달한 주문에만 다리 고리, 카드 "다른 학파로 통하는 길" 목록(클릭 이동), 성격 필터 막대. `easyMode.js`: 간단 모드에 트리 제작 방식 선택
