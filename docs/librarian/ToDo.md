...
- [x] 신규 `src/spellscanner/SpellScannerEvidence.cpp` — `effectDetails` 가 켜지면 붙는다.
  ...
  - 효과(MGEF): 플래그 전부, baseCost, minimumSkill, 투사체(종류·속도·사거리·중력·폭발 여부),
    폭발(반경, 효과 것인지 투사체 것인지), **장판 유무**, 붙은 퍽·능력, 상쇄 효과, 조건 유무
  - 주문 안의 효과: 순서 번호, cost, 조건 유무
...
- [ ] 기존 필드 회귀 확인 (`librarian-test` 숫자 불변 — 새 필드는 추가만 했다). 단, 이번 덤프는
      `keywordPatchApplied=true` 라 `MEASURED.md` 숫자와 직접 비교할 수 없다
- 부수 관찰: **M6 키워드 보정이 게임에서 처음 돌았다** — 후보 12588 이펙트 중 969개에 971개 추가,
  이미 있던 것 2032. 로드오더에 없는 키워드 3종(`MagicBoundWeapon` `MagicReanimate` `MagicSummonDaedra`)은
  비활성. 맞는 이펙트에만 붙었는지는 아직 재지 않았다
- 부수 관찰: `RunScan` 이 두 번 다 121~123초 걸렸다(M1 버그 2). 파일은 정상으로 써지지만
...
### [ ] S-UI. 주문 카드 (2026-09-21 사용자 스케치, 미착수)
