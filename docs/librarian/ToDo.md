...
- [ ] 게임 검증: 톰 모드 + full 스캔 →
  - 장판 **있음**이어야 하는 것: 화염의 벽 · 서리의 벽 · 폭풍의 벽 · 화염 룬
  - 장판 **없음**이어야 하는 것: 화염구 · 화염 화살
  - 화염구의 `projectile.type` = Missile, `explosion.radius` > 0
  - 바닐라 초보 주문의 `castingPerk` 가 초보 반값 퍽
  - 기존 필드 회귀 없음 (`librarian-test` 숫자 불변 — 새 필드는 추가만 했다)
