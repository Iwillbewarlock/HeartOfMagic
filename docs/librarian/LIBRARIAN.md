...
집어 태그를 **더한다**(빼지는 않는다). `persistentId` 는 플러그인 안의 ID라 로드오더와 무관하고,
설치되지 않은 주문은 그냥 매칭되지 않는다.

`80_manual.json` 은 1,440개 주문 로드오더의 스캔(2026-09-21)을 보고 만든 15개다:
...
에 두면 된다. 일부러 뺀 것: Natura 의 바위 주문들은 "출혈 피해"를 주지만 흙 마법이고, Mysticism 의
흡혈(Vampiric) 다섯 개는 에디터 ID 와 달리 엔트로피 마법이라 혈이 아니다 - 원소 없이 둔다.
